...
export default function CategoryDetailPage() {
  ...
  const { item, loading, error } = useResourceItem('categories', id);
  ...
  return (
    <>
      <PageHeader
        ...
        description={
          Array.isArray(item?.descriptionArray)
            ...
            : (item?.description ?? 'Category details.')
        }
        breadcrumbs={[
          { to: '/dashboard', label: 'Admin' },
          { to: '/categories', label: 'Categories' },
          { to: `/categories/${id}`, label: id ?? 'Record' },
        ]}
        action={
          item && (
            <Button type="button" onClick={() => navigate(`/categories/${item.id}/edit`)}>
              <Edit />
              Edit
            </Button>
          )
      ...
      <PageContainer>
        ...
        {error && <EmptyState title="Unable to load category" description={error} />}
        {!loading && !item && (
          <EmptyState
        ...
        {item && (
          <div className="grid gap-6 xl:grid-cols-3">
            <DashboardCard title="Category details" className="xl:col-span-2">
            ...
            <DashboardCard title="Subcategories">
              {Array.isArray(item.subCategories) && item.subCategories.length > 0 ? (
                <ul className="space-y-3">
                  {item.subCategories.map((subCat, index) => (
                    <li
                      key={typeof subCat === 'string' ? subCat : (subCat?.id ?? index)}
                      className="rounded-3xl border border-divider bg-surface-raised p-4"
                    >
                ...
                <p className="text-sm text-charcoal-light">No subcategories available.</p>
              )}
            </DashboardCard>
          </div>
        )}
